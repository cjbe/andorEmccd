...
class LibInstance:
    ...
    def __init__(self):
        ...
        self.get_available_cameras = wrapper(dll.GetAvailableCameras, [POINTER(c_long)])
    ...
    def lock_select_camera(self, camera_index):
        """Provide sole access to the library, guaranteeing the given camera remains
        selected during the 'with' block.

        When working with a single camera the GIL means we don't have to be careful about
        the multi-threading. However when several cameras are used other threads can
        change the selected camera underneath us."""
        self.lock.acquire()
        self.set_current_camera(camera_index)
        try:
            yield
        finally:
            self.lock.release()
...
def initialise_all_cameras():
    """Initialises all connected cameras.
    Returns a dictionary of AndorEmccd instances indexed by serial number
    """
    lib = LibInstance()

    n_cameras = c_long()
    lib.get_available_cameras(ctypes.byref(n_cameras))

    cameras = {}
    for i in range(n_cameras.value):
        handle = c_long()
        lib.get_camera_handle(i, ctypes.byref(handle))
        with lib.lock_select_camera(handle):
            # Initialize() needs to be called for every camera
            lib.initialize()

            serial = c_int()
            lib.get_camera_serial(ctypes.byref(serial))
        cameras[serial.value] = AndorEmccd(lib=lib, camera_handle=handle)
    return cameras
...
class AndorEmccd:
    ...
    def __init__(self, leave_camera_warm=True, framebuffer_len=100, lib=None,
            camera_handle=None):
        ...
        if camera_handle:
            self.lock_camera = lambda: self.lib.lock_select_camera(camera_handle)
        else:
            # We use suppress() here as nocontext() does not exist before python 3.7
    ...
    def set_preamp_gain(self, gain):
        ...
        self.preamp_gains"""
        try:
            index = self.preamp_gains.index(int(gain))
        except ValueError:
    ...
    def set_vertical_clock_voltage(self, boost):
        ...
        with self.lock_camera():
            self.lib.set_vs_amplitude(int(boost))
